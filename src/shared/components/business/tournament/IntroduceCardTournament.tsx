  ...
  return (
    <section id='IntroduceCardTournament' className={`relative w-full text-white rounded-lg shadow-lg ${className}`}>
      ...
      <div className='absolute bottom-0 w-full flex flex-col justify-center items-center gap-3 p-5'>
        ...
        <button className='font-semibold py-2 px-4 border-2 border-white rounded-md cursor-pointer shadow-2xl' onClick={() => router.push(`${URL_SYSTEMS.GROUP_TOURNAMENT}/${data.id}`)}>
          Xem các giải đấu
        </button>
      </div>
    </section>
